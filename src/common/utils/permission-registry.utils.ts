...
export async function handlePermissionRegistryAndReturnID(args: IArgs) {
	...
	if (config.app_key.includes("_dev")) {
		config.app_key = config.app_key.replace("_dev", "")
	}
	...
	// console.log("found_permission", found_permission, args)
	if (!found_permission) {
		// console.error(error)
	...
	let registry = SYNCED_CONFIG.path_registries.find(
		(e) =>
			...
			found_permission?.id === e.permission_id
	)
	if (!registry) {
		registry = await upsertPathRegistry({
			...args,
...
async function upsertPathRegistry(
	args: IArgs & { permission: typeof permissionsTable.$inferSelect }
) {
	const matrix_results = await SYNCED_CONFIG.__DANGEROURS_ACCESS_DB
		...
		.onConflictDoUpdate({
			target: [
				permissionPathRegistryTable.path,
				permissionPathRegistryTable.method
			],
			set: {
				updated_at: new Date(),
				description: args.config.description,
	...
	logger.success(
		`upserted pathpermission ${args.path} / ${args.method}`,
		JSON.stringify(inserted_matrix)
	)
...
async function createPermissionFromScratch(args: IArgs) {
	try {
		// logger.warning(
		// 	`upsert request for permission ${JSON.stringify(args,null,2)}`
		// )
		...
		await RELOAD_SYNCED_CONFIG({ db: SYNCED_CONFIG.__DANGEROURS_ACCESS_DB! })
		// console.log("returning",inserted_permission)
		return inserted_permission
	} catch (err) {
		logger.error("createPermissionFromScratch failed", err as any)
		return
	}
}
